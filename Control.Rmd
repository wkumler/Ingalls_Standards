...
Abbreviations <- Ingalls_Lab_Standards_LauraEdit %>%
  ...
  filter(nchar(Compound.Name_old) < 8,
         nchar(Compound.Name_old) < 4 | str_detect(Compound.Name_old, "-"))
```
...
internal.standards <- Ingalls.complete %>%
  filter(Compound.Type == "Internal Standard")

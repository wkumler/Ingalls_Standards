...
### Finalize changes of compounds - Including those that remain as the original name.
```{r Add unchanged compounds, include=TRUE}
Ingalls_Lab_Standards_ALL <- Ingalls_Lab_Standards_Symbols %>%
  left_join(Unchanged) %>%
  select(Compound.Name, Compound.Name_new, Compound.Name_old, Compound.Type, everything()) %>%
  mutate(Compound.Name = ifelse(is.na(Compound.Name), Compound.Name_new, Compound.Name)) %>%
...
write.csv(Ingalls_Lab_Standards_ALL, "Ingalls_Lab_Standards_NEW.csv")
```
------
